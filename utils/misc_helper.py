...
def to_device(input, device="cuda", dtype=None):
    ...
    if "pointcloud" in input:
        input["pointcloud"] = input["pointcloud"].to(dtype=dtype)
...
def update_config(config):
    ...
    if reconstruction_type == "UniAD":
        ...
        outstride = config.net[0].kwargs.outstrides[0]
        assert (
        ...
        config.net[1].kwargs.feature_size = feature_size
